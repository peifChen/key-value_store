...
hashnode_t *_create_node(char *key, char *value) {
	...
	hashnode_t *node = (hashnode_t*)kvserver_malloc(sizeof(hashnode_t));
	if (!node) return NULL;
...
int kvs_hash_create(kvs_hash_t *hash) {
	...
	hash->nodes = (hashnode_t**)kvserver_malloc(sizeof(hashnode_t*) * MAX_TABLE_SIZE);
...
int kvs_hash_del(kvs_hash_t *hash, char *key) {
	...
	if (strcmp(head->key, key) == 0) {
		...
		kvserver_free(head->key);
		kvserver_free(head->value);
		kvserver_free(head);
		hash->count --;
	...
	cur->next = tmp->next;
#if ENABLE_KEY_POINTER
	kvserver_free(tmp->key);
	kvserver_free(tmp->value);
#endif
	kvserver_free(tmp);

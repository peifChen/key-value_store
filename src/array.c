#include "kvstore.h"
#include <stdio.h>
#include <string.h>

// singleton
kvserver_array_t global_array = {0};

int kvserver_array_create(kvserver_array_t *inst) {
    if (!inst) return -1;
    if (inst->table) {
        printf("table has alloc\n");
        return -1;
    }
    inst->table = kvserver_malloc(KVS_ARRAY_SIZE * sizeof(kvserver_array_item_t));
    if (!inst->table) return -1;
    inst->total = 0;
    return 0;
}

void kvserver_array_destory(kvserver_array_t *inst) {
    if (!inst) return;
    if (inst->table) {
        kvserver_free(inst->table);
        inst->table = NULL;
    }
}

/*
 * @return: <0, error; =0, success; >0, exist
 */
int kvserver_array_set(kvserver_array_t *inst, char *key, char *value) {
    if (inst == NULL || key == NULL || value == NULL) return -1;
    if (inst->total == KVS_ARRAY_SIZE) return -1;

    char *str = kvserver_array_get(inst, key);
    if (str) return 1;

    char *kcopy = kvserver_malloc(strlen(key) + 1);
    if (kcopy == NULL) {
        printf("分配失败\n");
        return -2;
    }
    memset(kcopy, 0, strlen(key) + 1);
    strncpy(kcopy, key, strlen(key));

    char *kvalue = kvserver_malloc(strlen(value) + 1);
    if (kvalue == NULL) {
        printf("分配失败\n");
        kvserver_free(kcopy);
        return -2;
    }
    memset(kvalue, 0, strlen(value) + 1);
    strncpy(kvalue, value, strlen(value));

    int i = 0;
    for (i = 0; i < inst->total; i++) {
        if (inst->table[i].key == NULL) {
            inst->table[i].key = kcopy;
            inst->table[i].value = kvalue;
            inst->total++;
            return 0;
        }
    }
    if (i == inst->total && i < KVS_ARRAY_SIZE) {
        inst->table[i].key = kcopy;
        inst->table[i].value = kvalue;
        inst->total++;
    }
    return 0;
}

char* kvserver_array_get(kvserver_array_t *inst, char *key) {
    if (inst == NULL || key == NULL) return NULL;
    for (int i = 0; i < inst->total; i++) {
        if (inst->table[i].key == NULL) continue;
        if (strcmp(inst->table[i].key, key) == 0) {
            return inst->table[i].value;
        }
    }
    return NULL;
}

/*
 * @return < 0, error;  =0,  success; >0, no exist
 */
int kvserver_array_del(kvserver_array_t *inst, char *key) {
    if (inst == NULL || key == NULL) return -1;
    int i = 0;
    for (i = 0; i < inst->total; i++) {
        if (inst->table[i].key && strcmp(inst->table[i].key, key) == 0) {
            kvserver_free(inst->table[i].key);
            inst->table[i].key = NULL;
            kvserver_free(inst->table[i].value);
            inst->table[i].value = NULL;
            if (inst->total - 1 == i) inst->total--;
            return 0;
        }
    }
    return i;
}

/*
 * @return : < 0, error; =0, success; >0, no exist
 */
int kvserver_array_mod(kvserver_array_t *inst, char *key, char *value) {
    if (inst == NULL || key == NULL || value == NULL) return -1;
    if (inst->total == 0) return KVS_ARRAY_SIZE;

    for (int i = 0; i < inst->total; i++) {
        if (inst->table[i].key == NULL) continue;
        if (strcmp(inst->table[i].key, key) == 0) {
            kvserver_free(inst->table[i].value);
            char *kvalue = kvserver_malloc(strlen(value) + 1);
            if (kvalue == NULL) return -2;
            memset(kvalue, 0, strlen(value) + 1);
            strncpy(kvalue, value, strlen(value));
            inst->table[i].value = kvalue;
            return 0;
        }
    }
    return inst->total;
}

/*
 * @return 0: exist, 1: no exist
 */
int kvserver_array_exist(kvserver_array_t *inst, char *key) {
    if (!inst || !key) return -1;
    char *str = kvserver_array_get(inst, key);
	if(!str) printf("Error_!str!\n");
    return str ? 0 : 1;
}